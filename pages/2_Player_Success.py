...
years = sorted(df["season"].dropna().unique(), reverse=True)
years = [y for y in years if y != 2025]
selected_year = st.sidebar.selectbox("Select Year", years, index=years.index(2020))
...
x_range = [1, 6]  # Since draft rounds are 1-indexed and we're limiting to first 6 rounds
y_range = [0, team_stats["avg_wav"].max() * 1.1]
x_mid = sum(x_range) / 2
y_mid = sum(y_range) / 2

# --- Build Team-Level Quadrant Plot ---
fig = px.scatter(
    team_stats,
    x="avg_round",
    y="avg_wav",
    ...
    opacity=0  # make the underlying points invisible
)

# --- Add logo images and hover traces ---
for _, row in team_stats.iterrows():
    ...
    fig.add_trace(
        px.scatter(x=[row["avg_round"]], y=[row["avg_wav"]], hover_name=[hover_text]).data[0]
    )
...
st.markdown("This chart compares NFL teams based on average draft round and weighted AV (W_AV) of their draftees. Quadrants provide interpretive guidance.")
